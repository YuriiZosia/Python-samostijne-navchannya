...
def check_Pythagorean_trykutnyk(input_string):
  ...
  numbers_str = input_string.split(',')
  numbers = [float(num) for num in numbers_str]
  numbers.sort(reverse=True) # вістортуємо по спадінню, щоб більше число (гіпотенуза) була перша в масиві
  c, a, b = numbers # c - гіпотенуза
  return a**2 + b**2 == c**2

print("Програма, яка приймає масив несортованих чисел і повертає boolean значення залежно від того,чи можна із заданих значень скласти піфагорів трикутник з відповідними довжинами сторін")
